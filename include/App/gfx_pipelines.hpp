...
create_mesh_pipeline(arena_t* arena, state_t* state, const mesh_pipeline_info_t& buffers) {
    ...
    create_info->vertex_shader = "./assets/shaders/bin/simple.vert.spv";
    create_info->fragment_shader = "./assets/shaders/bin/simple.frag.spv";
    ...
    //todo(zack): fill out uniform buffers
    VkDescriptorBufferInfo buffer_info[3];
    buffer_info[0].buffer = buffers.sporadic_buffer;
    buffer_info[0].offset = 0; 
    buffer_info[0].range = sizeof(sporadic_buffer_t);

    buffer_info[1].buffer = buffers.scene_buffer;
    buffer_info[1].offset = 0; 
    buffer_info[1].range = sizeof(scene_buffer_t);

    buffer_info[2].buffer = buffers.object_buffer;
